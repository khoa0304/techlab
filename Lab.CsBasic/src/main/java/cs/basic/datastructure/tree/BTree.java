...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    private static final class Node {
        ...
        private Entry[] children = new Entry[MAX_CHILDREN_PER_NODE];   // the array of children
    ...
    private Node insert(Node node, Key key, Value val, int height) {
        ...
        if (node.numberofChildren < MAX_CHILDREN_PER_NODE) return null;
        else         return split(node);
    }

    // split node in half
    private Node split(Node h) {
        Node t = new Node(MAX_CHILDREN_PER_NODE/2);
        h.numberofChildren = MAX_CHILDREN_PER_NODE/2;
        for (int j = 0; j < MAX_CHILDREN_PER_NODE/2; j++)
            t.children[j] = h.children[MAX_CHILDREN_PER_NODE/2+j]; 
        return t;    
    }
