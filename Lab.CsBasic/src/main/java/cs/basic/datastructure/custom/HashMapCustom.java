...
/**
 * 
 * Copy From https://www.javamadesoeasy.com/2015/02/hashmap-custom-implementation.html
 *
 * @param <K>
 * @param <V>
 */
class HashMapCustom<K, V> {
    ...
    private Entry<K,V>[] entryArray;   //Array of Entry.
   ...
   public void put(K newKey, V data){
      ...
      //if table location does not contain any entry, store entry there.
       if(entryArray[hash] == null){
        entryArray[hash] = newEntry;
       }else{
          Entry<K,V> previous = null;
          Entry<K,V> current = entryArray[hash];
   ...
   public V get(K key){
       ...
       if(entryArray[hash] == null){
        ...
        Entry<K,V> temp = entryArray[hash];
        while(temp!= null){
       ...
       while(current != null){ //we have reached last entry node of bucket.
          if(current.key.equals(deleteKey)){               
              if(previous==null){  //delete first entry node.
                    entryArray[hash]=entryArray[hash].next;
                    return true;
              }
              else{
